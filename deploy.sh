...
Options:
    ...
    -r, --github-repo <GITHUB_REPO> GitHub repo with username,
                                         default to \"geodirectory/geodirectory\".
    -p, --default-package <NAME>     Default package name,
                                         default to \"geodirectory\".
...
parse_args() {
    ...
    if [[ -z $github_repo ]]; then
        github_repo="Ayecode/geodirectory"
    fi

    if [[ -z $default_package ]]; then
        default_package="Geodirectory"
    fi
...
download_source() {
    ...
    output 3 "https://github.com/${github_repo}/archive/refs/tags/${source_version}.zip"
    echo
    #curl -LSO# "https://github.com/${github_repo}/releases/download/${source_version}/${project_name}.zip"
    curl -LSO# "https://github.com/${github_repo}/archive/refs/tags/${source_version}.zip" -o "${project_name}.zip"
