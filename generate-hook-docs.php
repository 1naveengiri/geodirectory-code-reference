...
class HookDocsGenerator
{
     ...
     * Source path.
     */
    protected const SOURCE_PATH = 'geodirectory/';
    ...
    protected static function getFilesToScan(): array
    {
       ...
       # $files['Data Stores']        = self::getFiles('*.php', GLOB_MARK, self::SOURCE_PATH . 'includes/data-stores');
        $files['Core Classes']       = array_merge(
            ...
            self::getFiles('*.php', GLOB_MARK, self::SOURCE_PATH . 'includes/admin/'),
            self::getFiles('*.php', GLOB_MARK, self::SOURCE_PATH . 'includes/api/'),
            self::getFiles('*.php', GLOB_MARK, self::SOURCE_PATH . 'includes/custom-fields/'),
            self::getFiles('*.php', GLOB_MARK, self::SOURCE_PATH . 'includes/elementor/'),
            self::getFiles('*.php', GLOB_MARK, self::SOURCE_PATH . 'includes/libraries/'),
        );
        ...
        foreach ($hook_list as $heading => $hooks) {
            foreach ($hooks as $hook => $details) {
                ...
                foreach ($details['files'] as $file) {
                    ...
                    $output .= 'url: "' . str_replace('../', 'https://1naveengiri.github.io/geodirectory-code-reference/', self::getFileURL($file)) . '"';
                    $output .= '}';
                }
            }
        }
